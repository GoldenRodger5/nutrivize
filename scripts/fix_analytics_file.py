...
This script creates a corrected analytics.py file
"""

import sys
import os

# Corrected analytics.py content
ANALYTICS_CONTENT = """from fastapi import APIRouter, Depends, HTTPException, Query
...
def install_fixed_analytics():
    """Install the fixed analytics.py file"""
    
    analytics_path = "./backend/app/routes/analytics.py"
    
    if not os.path.exists(os.path.dirname(analytics_path)):
        print(f"Error: Directory not found: {os.path.dirname(analytics_path)}")
        return False
    
    try:
        # Create backup
        if os.path.exists(analytics_path):
            backup_path = f"{analytics_path}.bak.{datetime.now().strftime('%Y%m%d%H%M%S')}"
            with open(analytics_path, 'r') as src, open(backup_path, 'w') as dst:
                dst.write(src.read())
        ...
        print(f"✅ Successfully installed fixed analytics.py file")
        return True
    
    except Exception as e:
        print(f"Error installing fixed analytics.py: {e}")
        return False


if __name__ == "__main__":
    # Install fixed analytics.py
    if install_fixed_analytics():
        sys.exit(0)
    else:
        sys.exit(1)
