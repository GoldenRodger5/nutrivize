...
async def get_user_favorites_alias(
    current_user: UserResponse = Depends(get_current_user)
):
    """Get user's favorite foods - Direct alias for production endpoint"""
    try:
        from ..services.user_favorites_service import user_favorites_service
        ...
        return favorites  # Return direct list to match /favorites endpoint format
        
    except Exception as e:
        logger.error(f"Error getting user favorites: {str(e)}")
        raise HTTPException(status_code=500, detail="Failed to get user favorites")
